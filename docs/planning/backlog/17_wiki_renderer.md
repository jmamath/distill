...
**Depends on:** Plan 19 (experiment-grounded finding contract); Plan 9 (durable graph-update outcomes and authoritative theme placement); Plan 13 (LLM-as-judge helper for the quality gate).
...
```mermaid
flowchart TB
    signal[("Plan 19 signal<br/>experimental findings")]:::data
    graph["Plan 9 · interpret finding<br/>attach · open · drop"]:::upstream
    outcome[("Durable graph outcome<br/>evidence · hypothesis<br/>theme placement + source provenance")]:::data
...
Plan 17 does not match evidence to hypotheses, assign themes, reopen source signals, or reconsider Plan 9's disposition. Those are graph decisions. It may read the evidence, hypothesis, and source provenance referenced by an outcome to write accurate prose.
...
- wholly new creates one anchored section.

Writes are idempotent by graph-outcome id plus destination theme. Replaying Plan 9 or restarting Plan 17 cannot duplicate prose, anchors, or decision rows.

Likely implementation areas—exact files pinned at `doing`—are the renderer entry point, stable-anchor helpers, the renderer decision/progress store, and focused tests.
...
A small golden set pairs graph outcomes and theme bodies with human novelty verdicts. It must cover attached evidence, a newly opened hypothesis, a mixed-paper sequence, and a multi-theme non-redundancy case. The eval reuses Plan 13's judge helper and names incorrect verdicts in a readable report.
...
- Evidence is rendered under the matched hypothesis's themes; a new hypothesis uses the themes assigned at creation.
...
- Reprocessing an outcome is idempotent across prose and decisions.
...
- Reconsidering Plan 9's attach / open / drop or stance decisions.
- Updating `entities.json` or `timeline.json` from incoming non-experimental facts; that ingestion path is deferred beyond the first version.
